...
export function convertTrait(initial) {
	...
	trait = trait.join(":");
	switch (type) {
		case "saves": trait = convertAbility(trait); break;
		case "skills": trait = convertSkill(trait); break;
		case "languages": trait = convertLanguage(trait); break;
		case "armor": trait = convertArmor(trait); break;
		case "weapon": trait = convertWeapon(trait); break;
		case "tool": trait = convertTool(trait); break;
		// TODO: di, dr, dv, ci
	}
	return `${type}:${trait}`;
}
